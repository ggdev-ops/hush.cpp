...
import myai.hush.engine.HushRecorder
...
class AppViewModel(
    ...
    private var activeRecorder: HushRecorder? = null
    ...
    fun toggleRecording() {
        viewModelScope.launch {
            val currentRecorder = activeRecorder
            if (currentRecorder != null && _state.value is AppState.Recording) {
                ...
                _lastReduction.value = null
                try {
                    val rec = HushRecorder(
                        outputFile = path,
                        thresholdDb = _selectedPreset.value.threshold,
                        aggressionLevel = _selectedPreset.value.aggression,
                        sampleRate = 16000,
                        useSilenceRemoval = false // Record raw audio first for App's post-processing Hush workflow
                    )
                    if (rec.start()) {
                        activeRecorder = rec
                        ...
                        rec.close()
                        _state.value = AppState.Error("Failed to start native recording device")
                    }
                } catch (e: Exception) {
                    _state.value = AppState.Error(e.message ?: "Failed to initialize native recorder")
                }
            }
        }
    }
